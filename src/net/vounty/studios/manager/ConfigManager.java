...
public class ConfigManager {
    ...
    public <T> T readConfigFromWeb(String url, Class<T> type) {
        try {
            InputStreamReader inputStreamReader = new InputStreamReader(new URL(url).openStream(), StandardCharsets.UTF_8);
            return new Gson().fromJson(inputStreamReader, type);
        } catch (Exception exception) {
            exception.printStackTrace();
            return null;
